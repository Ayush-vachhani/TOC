<script lang="ts">
  ...
  import type { DefaultEdgeOptions, Edge, SvelteFlowStore } from '@xyflow/svelte';

  import NormalNode from '$lib/NormalNode.svelte';
  import StartNode from '$lib/StartNode.svelte';
  import StarNode from '$lib/StarNode.svelte';
  import OrNode from '$lib/OrNode.svelte';
...
<div style:height="500px">
	...
	<input type="field" id="add-string"/>
	<input type="button" class = "insert-node" value="Add string match" on:click={()=>{
	let e = document.getElementById('add-string')
	insertNode(e.value)
	}} />
	<br>
	<input type="button" class = "insert-node" value="Match Multiple Times" on:click={()=>{insertStarNode()}} />
	<input type="button" class = "insert-node" value="Match Any One" on:click={()=>{insertOrNode()}} />
	<br>
	<input type="field" id="regex-result"/>
	<input type="button" class="get-regex" value="Convert to RegEx" on:click={convertHandler} />
</div>
